...
async def _ask_talon(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    profile = await db.get_profile(update.effective_user.id)
    current = profile.get("talon_no") or "—" if profile else "—"

    await update.message.reply_html(
        "📋 <b>Enrollment Wizard</b> — Step 4 of 4\n\n"
        f"Current Talon No: <code>{current}</code>\n\n"
        "Please enter your <b>Talon number</b> (small registration card, 6–12 digits).\n"
        "Send /skip to keep the current value, or /cancel to quit."
    )
    return ASK_TALON
...
async def received_talon(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not _TALON_RE.match(text):
        await update.message.reply_text(
            "❌ Invalid talon number (6–12 digits).  Try again or /skip."
        )
        return ASK_TALON
...
async def skip_talon(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    context.user_data.pop("enroll_talon", None)
    return await _save_and_confirm(update, context)
